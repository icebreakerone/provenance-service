...
From https://github.com/g-a-d/aws-kms-sign-csr/blob/master/aws-kms-sign-csr.py
"""
...
def signing_algorithm(hashalgo, signalgo):
    ...
    if hashalgo == "sha512" and signalgo == "ECDSA":
        ...
        raise Exception(
            "unknown hash algorithm, please specify one of sha224, sha256, sha384, or sha512"
        )
...
def main(args):
    with open(args.csr, "r") as f:
        substrate = pyasn1_modules.pem.readPemFromFile(
            f, startMarker=start_marker, endMarker=end_marker
        )
        csr = decoder.decode(
    ...
    signatureBytes = sign_certification_request_info(
        kms,
        args.keyid,
        csr,
        args.hashalgo,
